...
postingAsLines elideamount onelineamounts pstoalignwith p = concat [
  ...
  where
    ...
    shownAmounts
      | elideamount    = [""]
      | onelineamounts = [fitString (Just (amtwidth - acctwidth)) Nothing False False $ showMixedAmountOneLine $ pamount p]
      | null (amounts $ pamount p) = [""]
      | otherwise      = map (fitStringMulti (Just (amtwidth - acctwidth)) Nothing False False . showAmount ) . amounts $ pamount p
      where
        amtwidth = maximum $ 42 : map (strWidth . showMixedAmount . pamount) pstoalignwith  -- min. 12 for backwards compatibility
        acctwidth = maximum $ map (textWidth . paccount) pstoalignwith
